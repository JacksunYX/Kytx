...
- (void)showErrorPageView{
    ...
    if (!self.errorPageView) {
        self.errorPageView = [[DWQErrorPageView alloc]initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT-64)];
...
- (void)showBlankPageView{
    if (!self.blankPageView) {
        self.blankPageView = [[DWQBlankPageView alloc]initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT-64)];
    }
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        errorTipLabel.text = @"虾米夺宝提醒：您的网络好像有点问题哦~";
...
- (void)_clickReloadButton:(UIButton* )btn{
    ...
    if (_didClickReloadBlock) {
        ...
        [_errorImageView setImage:[UIImage imageNamed:@"loading_imgBlue_78x78"]];
        ...
        [_errorImageView.layer addAnimation:animation forKey:@"transform"];
...
- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        UIImageView* nodataImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"loading_imgBlue_78x78"]];
        _nodataImageView = nodataImageView;
        [self addSubview:_nodataImageView];
        
        UILabel* nodataTipLabel = [[UILabel alloc]init];
        nodataTipLabel.numberOfLines = 2;
        ...
        nodataTipLabel.text = @"虾米夺宝提醒：这里没有数据呢,赶紧弄出点动静吧~";
        ...
        [_nodataTipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(_nodataImageView.mas_bottom).offset(15);
        }];
    }
    return self;
}
