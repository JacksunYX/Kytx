...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    if (self.borderType & HMSegmentedControlBorderTypeCustomer) {
        UIEdgeInsets borderInsets = self.customerBordrInsetBackground;
        CALayer *borderLayer = [CALayer layer];
        borderLayer.frame = CGRectMake(borderInsets.left, self.scrollView.frame.size.height -self.borderWidth-borderInsets.bottom, self.scrollView.frame.size.width - borderInsets.left - borderInsets.right, self.borderWidth);
        borderLayer.backgroundColor = self.borderColor.CGColor;
//        self.scrollView.backgroundColor = [UIColor redColor];
        [self.scrollView.layer addSublayer: borderLayer];
    }
