...
@property (nonatomic, strong) UIButton *cancelBtn;  //自定义的取消按钮
@end
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (NO == self.navigationController.navigationBar.translucent) {
        ...
        self.searchSuggestionVC.view.frame = CGRectMake(0, 64 - self.view.py_y + 50, self.view.py_width, self.view.py_height + self.view.py_y - 50);
...
+ (instancetype)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder andsearchHeadViewHidden:(NSString *)searchHeadViewHidden
{
    PYSearchViewController *searchVC = [[PYSearchViewController alloc] init];
    searchVC.searchheadviewHidden=searchHeadViewHidden;
    [searchVC setup];
...
+ (instancetype)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder andsearchHeadViewHidden:(NSString *)searchHeadViewHidden didSearchBlock:(PYDidSearchBlock)block
{
    PYSearchViewController *searchVC = [self searchViewControllerWithHotSearches:hotSearches searchBarPlaceholder:placeholder andsearchHeadViewHidden:searchHeadViewHidden];
...
//                [_swSelf searchBarSearchButtonClicked:_swSelf.searchBar];
                if ([self.delegate respondsToSelector:@selector(searchViewController:didSearchWithSearchBar:searchText:)]) {
                    [self.delegate searchViewController:self didSearchWithSearchBar:_swSelf.searchBar searchText:_swSelf.searchBar.text];
                    [self saveSearchCacheAndRefreshView];
                    return;
                }
                if (self.didSearchBlock) self.didSearchBlock(self, _swSelf.searchBar, _swSelf.searchBar.text,searchtypes,@"isSearchCellClick");
                [self saveSearchCacheAndRefreshView];
            }
        };
...
//    [_cancelBtn addTarget:self action:@selector(cancelDidClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_cancelBtn.titleLabel setTextAlignment:NSTextAlignmentRight];
    [_cancelBtn.titleLabel setFont:PFR15Font];
    _cancelBtn.size = CGSizeMake(40, 40);
    //影响按钮内的titleLable;
    _cancelBtn.titleEdgeInsets=UIEdgeInsetsMake(0, 0, 0, 0);
    ...
    NSLog(@"点击了-----%@",self.searchheadviewHidden);
    self.searchheadview=[self.searchheadviewHidden isEqualToString:@"YES"]?[[SearchHeadView alloc]initWithFrame:CGRectMake(0, -50, SCREEN_WIDTH, 0)]:[[SearchHeadView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 50)];
    __weak typeof(self) weakself = self;
    self.searchheadview.searchheadClickBlock = ^(NSString *strValue) {
        NSLog(@"点击了-----%@",strValue);
        searchtypes=strValue;
        [weakself.searchBar setPlaceholder:[NSString stringWithFormat:@"搜索%@",strValue]];
    };
...
- (UILabel *)setupTitleLabel:(NSString *)title
{
    ...
    titleLabel.py_x = 0;
    titleLabel.py_y = self.searchheadview.height;
    return titleLabel;
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if (self.didSearchBlock) self.didSearchBlock(self, searchBar, searchBar.text,searchtypes,@"isSearchButtonClick");
    [self saveSearchCacheAndRefreshView];
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if ([self.delegate respondsToSelector:@selector(searchViewController:searchTextDidChange:searchText:searchType:)]) {
        [self.delegate searchViewController:self searchTextDidChange:searchBar searchText:searchText searchType:searchtypes];
    }
}
