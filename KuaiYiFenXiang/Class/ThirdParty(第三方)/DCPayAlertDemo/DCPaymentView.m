...
- (void)drawView {
    if (!_paymentAlert) {
        ...
        if (kStringIsEmpty(self.detail)) {
            
            _paymentAlert = [[UIView alloc]initWithFrame:CGRectMake(40, [UIScreen mainScreen].bounds.size.height-KEYBOARD_HEIGHT-KEY_VIEW_DISTANCE-ALERT_HEIGHT-BOTTOM_MARGIN, [UIScreen mainScreen].bounds.size.width-80, ALERT_HEIGHT)];
...
//            [_closeBtn setFrame:CGRectMake(ScaleWidth(255), 0, TITLE_HEIGHT, TITLE_HEIGHT)];
            [_closeBtn setTitle:@"╳" forState:UIControlStateNormal];
            [_closeBtn setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
            [_closeBtn addTarget:self action:@selector(dismiss2) forControlEvents:UIControlEventTouchUpInside];
            _closeBtn.titleLabel.font = [UIFont systemFontOfSize:15];
            [_paymentAlert addSubview:_closeBtn];
            [_closeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.size.mas_equalTo(CGSizeMake(TITLE_HEIGHT, TITLE_HEIGHT));
                make.top.equalTo(_paymentAlert);
                make.right.equalTo(_paymentAlert).offset(-5);
            }];
            ...
            _inputView = [[UIView alloc]initWithFrame:CGRectMake(15, _line.y+40, PAYMENT_WIDTH-30, (PAYMENT_WIDTH-30)/6)];
...
//            _pwdTextField.keyboardType = UIKeyboardTypeNumberPad;
            ...
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.top.equalTo(_pwdTextField.mas_bottom).offset(15);
...
- (void)complete:(UIButton *)sender {
    if (_completeHandle) {
        _completeHandle(self.pwdTextField.text);
    }
    [self performSelector:@selector(dismiss) withObject:nil afterDelay:.3f];
}
