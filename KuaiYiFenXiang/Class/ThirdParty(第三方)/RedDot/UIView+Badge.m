...
-(void)yee_MakeRedBadge:(CGFloat)corner color:(UIColor *)cornerColor{
  ...
  //圆点颜色
    ...
    [[self yee_BadgeLable]setFrame:CGRectMake(self.frame.size.width-corner*2, corner/6, corner*2.0, corner*2.0)];
...
-(void)yee_MakeBadgeText:(NSString *)text
    ...
    CGSize textSize=[self sizeWithString:text font:tfont constrainedToWidth:self.frame.size.width];
        if ([self isKindOfClass:[UIButton class]]) {
            ...
            [[self  yee_BadgeLable] makeBrdgeViewWithText:text textColor:tColor backColor:backColor textFont:tfont tframe:CGRectMake(weakButton.width*0.6,weakButton.imageView.frame.origin.y, textSize.width+8.0, textSize.height)];
        }else{
