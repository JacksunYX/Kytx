...
@interface FocusOnStoreViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation FocusOnStoreViewController{
...
-(void)loadNewData{
    ...
    [HttpRequest postWithTokenURLString:NetRequestUrl(collectShoplist) parameters:dic
                                    ...
                                    if (kArrayIsEmpty(result)&&kArrayIsEmpty(self.dataSource)) {
                                        ...
                                        if (codeStr.intValue==1) {
                                            ...
                                            for (NSDictionary * dic in result) {
                                                ...
                                                NSMutableArray *goodsarray=[[dic objectForKey:@"goods"] mutableCopy];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //通过cell的类方法直接初始化cell
    TheStoreTableViewCell *cell = [TheStoreTableViewCell mainTableViewCellWithTableView:tableView];
    
    //根据相应的cell的行数进行每行cell的数据设置
    kArrayIsEmpty(self.dataSource)?:[cell setTheStoreModel:self.dataSource[indexPath.section]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    StoreDisplayViewController *sdvc=[[StoreDisplayViewController alloc]init];
    TheStoreModel *model = self.dataSource[indexPath.section];
    sdvc.TheStoreModel = model;
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"取消\n关注" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        TheStoreModel *model = self.dataSource[indexPath.row];
        // 设置请求参数可变数组
        NSMutableDictionary *dic = [NSMutableDictionary new];
        
        [dic setObject:model.business_id forKey:@"shop_id"];
