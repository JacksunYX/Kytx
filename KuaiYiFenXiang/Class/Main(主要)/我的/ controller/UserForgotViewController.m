...
#import "UserForgotViewController.h"
#import "UserForgot2ViewController.h"
#import "KYHeader.h"

@interface UserForgotViewController ()
...
@implementation UserForgotViewController
...
- (void)next:(UIButton *)sender {
    UserForgot2ViewController *vc = [UserForgot2ViewController new];
    vc.code = self.tf2.text;
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)getCode:(UIButton *)sender {
    
    [HttpRequest postWithURLString:NetRequestUrl(sendValidate) parameters:[@{@"type" : @"2", @"send" : self.mobile} mutableCopy] isShowToastd:YES isShowHud:NO isShowBlankPages:NO success:^(id res) {
        //        NSLog(@"%@", res);
