...
#import "ShopUploadTableViewCell.h"
#import "GoPayViewController.h"
#import "ExampleViewController.h"
#import "StoreManagementViewController.h"

@interface ShopRegisterViewController ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, TZImagePickerControllerDelegate, UINavigationControllerDelegate> {
    NSString *_name;
    NSString *_phone;
    NSString *_idcard;
    NSString *_shopname;
...
- (void)configUI {
    ...
    [_table registerClass:[ShopUploadTableViewCell class] forCellReuseIdentifier:@"cell3"];
...
- (void)submit {
    ...
    //    KYInfoTableViewCell *cell = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
    name = _name;
    
    //    KYInfoTableViewCell *cell2 = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]];
    phone = _phone;
    idcard = _idcard;
    //    KYInfoTableViewCell *cell3 = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
    //    idcard = _idcard;
    ...
    //    KYInfoTableViewCell *cell4 = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0]];
    shopname = _shopname;
    
    //    KYInfoTableViewCell *cell5 = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:5 inSection:0]];
    ...
    //    KYInfoTableViewCell *cell6 = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:7 inSection:0]];
    address = _address;
    ...
    ShopUploadTableViewCell *cell7 = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:9 inSection:0]];
    ...
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        NSData *data1 = UIImageJPEGRepresentation(logoimage, 1.0f);
        NSString *encodedImageStr1 = [data1 base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
        dict[@"photo"] = [@"data:image/jpeg;base64," stringByAppendingString:encodedImageStr1];
        ...
        dict[@"license"] = [@"data:image/jpeg;base64," stringByAppendingString:encodedImageStr2];
        
        NSData *data3 = UIImageJPEGRepresentation(fimage, 1.0f);
...
- (void)putImage:(UIImage *)image atIndex:(NSInteger)index {
    ShopUploadTableViewCell *cell = [self.table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:9 inSection:0]];
    UIButton *btn;
    if (index == 1) {
        btn = [cell btn1];
        self.hasPhoto1 = YES;
...
//                self.addressF = cell.holder;
            ...
            ShopUploadTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell3" forIndexPath:indexPath];
            MCWeakSelf
    ...
    if ([tableView isEqual:self.table1]) {
        ...
        if (!self.sArray.count) {
            ...
            if (indexPath.row == 0) {
                ...
                cell.holder.text = [[self.pmodel.title stringByAppendingString:@"   "] stringByAppendingString: self.sArray[indexPath.row].title];                [self tap3:nil];
            }
...
//        kShowHUDAndActivity;
        ...
        if (!kStringIsEmpty(self.dict[@"logo"])) {
            [btn1 sd_setImageWithURL:[NSURL URLWithString:[defaultUrl stringByAppendingString:self.dict[@"logo"]]] forState:UIControlStateNormal completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                i++;
        ...
        UIButton *btn2 = [cell7 btn2];
        if (!kStringIsEmpty(self.dict[@"license"])) {
            [btn2 sd_setImageWithURL:[NSURL URLWithString:[defaultUrl stringByAppendingString:self.dict[@"license"]]] forState:UIControlStateNormal completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                i++;
        ...
        UIButton *btn3 = [cell7 btn3];
        if (!kStringIsEmpty(self.dict[@"photo"])) {
            [btn3 sd_setImageWithURL:[NSURL URLWithString:[defaultUrl stringByAppendingString:self.dict[@"photo"]]] forState:UIControlStateNormal completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                i++;
...
- (NSArray *)titleArray {
    if (!_titleArray) {
        _titleArray = @[@"店铺管理人姓名：", @"店铺管理人手机号：",@"管理人身份证号：", @"行业类型：", @"店铺名称：", @"店铺电话：", @"店铺区域：", @"详细地址：", @"营业时间：", @"上传店铺logo："];
    }
    return _titleArray;
}
...
- (NSArray *)placeHolderArray {
    if (!_placeHolderArray) {
        _placeHolderArray = @[@"请输入管理人姓名", @"请输入管理人手机号",@"请输入身份证号", @"请选择行业/分类", @"自定义+企业店", @"请输入您的店铺电话", @"请选择店铺区域", @"请输入您的详细地址", @"请选择营业时间", @""];
    }
