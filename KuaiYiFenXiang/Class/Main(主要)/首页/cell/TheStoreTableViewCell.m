...
@interface TheStoreTableViewCell ()
...
@implementation TheStoreTableViewCell

+(instancetype)mainTableViewCellWithTableView:(UITableView *)tableView{
    //防止cell重建引起卡顿
    // 定义唯一标识
    static NSString *CellIdentifier = @"TheStoreTableViewCell";
    // 通过唯一标识创建cell实例
    TheStoreTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    // 判断为空进行初始化  --（当拉动页面显示超过主页面内容的时候就会重用之前的cell，而不会再次初始化）
    if (!cell) {
        cell = [[TheStoreTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
...
-(void)setTheStoreModel:(TheStoreModel *)TheStoreModel{
    ...
    [self.storeLogoImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",DefaultDomainName,TheStoreModel.logo]] placeholderImage:[UIImage imageNamed:@"applogo"]];
    ...
    [self.shopIntroductionLabel setText:[NSString stringWithFormat:@"销量:%@件  共%@件",TheStoreModel.sell_total,TheStoreModel.good_num]];
    if (TheStoreModel.shopPictureArray.count >= 1) {
        kStringIsEmpty(TheStoreModel.shopPictureArray[0])?:[self.shopPictureImageView1 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",DefaultDomainName,TheStoreModel.shopPictureArray[0]]] placeholderImage:[UIImage imageNamed:@"applogo"]];
    }
    
    if (TheStoreModel.shopPictureArray.count >= 2) {
        kStringIsEmpty(TheStoreModel.shopPictureArray[1])?:[self.shopPictureImageView2 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",DefaultDomainName,TheStoreModel.shopPictureArray[1]]] placeholderImage:[UIImage imageNamed:@"applogo"]];
    }
    
    if (TheStoreModel.shopPictureArray.count >= 3) {
        kStringIsEmpty(TheStoreModel.shopPictureArray[2])?:[self.shopPictureImageView3 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",DefaultDomainName,TheStoreModel.shopPictureArray[2]]] placeholderImage:[UIImage imageNamed:@"applogo"]];
    }
...
- (UIButton *)enterStoreButton
{
    ...
    _enterStoreButton.titleLabel.font=PFR10Font;
