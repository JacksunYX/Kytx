...
- (void)setupView {
    ...
    [self.shopcartHeaderBgView addSubview:self.brandImageview];
...
-(UIImageView *)brandImageview {
    if (_brandImageview == nil){
        ...
        [_brandImageview setImage:[UIImage imageNamed:@"dianpu"]];
    }
...
- (void)layoutSubviews {
    ...
    [self.brandImageview mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.allSelectButton).offset(30);
        make.centerY.equalTo(self.shopcartHeaderBgView);
        make.size.mas_equalTo(CGSizeMake(15, 15));
    }];

    [self.brandLable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.brandImageview).offset(20);
