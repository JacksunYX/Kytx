...
{
    //加入购物车或点击直接购买通知
    _dcObj = [[NSNotificationCenter defaultCenter]addObserverForName:@"TheRequestDataSuccessfullyPassed" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        _RootAttributeArray=note.userInfo[@"RootAttributeArray"];
        _Spec_goods_priceArray=note.userInfo[@"Spec_goods_priceArray"];
        _goods_infodictionary=note.userInfo[@"goods_infodictionary"];
    }];
    ...
    _dcObj = [[NSNotificationCenter defaultCenter]addObserverForName:@"callphone" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        [callWebview loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:phonenumstring]]];
