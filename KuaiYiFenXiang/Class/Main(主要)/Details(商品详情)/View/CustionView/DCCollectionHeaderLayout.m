...
- (instancetype)init {
    if (self = [super init]) {
        ...
        if (ScreenW<=320&&ScreenH<=568) {
            self.labelFont = PFR10Font;
        }else{
            self.labelFont = PFR12Font;
        }
    }
    return self;
}
...
//    NSLog(@"字符串的长度：%ld",content.length);
    CGSize size = [content sizeWithAttributes:@{NSFontAttributeName:self.labelFont}];
    
    CGFloat itemWidth = MAX(size.width, self.itemHeight);

    //这里没有处理itemWidth大于屏幕宽度的情况
    CGRect sizeToFit = [content boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, self.itemHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.labelFont} context:nil];
    CGFloat w = sizeToFit.size.width;
    CGFloat h = sizeToFit.size.height;
//    NSLog(@"宽度：%lf  高度：%lf",w,h);
    if (h >= w) {   //高度大于宽度的情况
        sizeToFit.size.width = w + 20;
    }else{  //宽度大于高度的情况
        if (sizeToFit.size.width >= ScreenW - 20) {
            sizeToFit.size.width = ScreenW - 20;
        }else{
            sizeToFit.size.width += 20;
        }
    }
    
//    NSLog(@"最终宽度：%lf  最终高度：%lf",sizeToFit.size.width,sizeToFit.size.height);
